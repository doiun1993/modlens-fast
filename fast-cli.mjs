...
// openai 活跃槽（默认阿里云，用户可用 modlens config use 切换），
// 直接把用户问题连同图片发给视觉模型，返回一句聚焦答案。
// 间歇性卡顿对策：递增重试——第 1 次 10 秒、第 2 次 20 秒、第 3 次 30 秒…
// 超时即换新连接重发，总预算 90 秒，厂商池级故障通常在某一次重试中恢复。
//
...
function parseArgs(argv) {
  const out = { input: '', prompt: '', timeout: 90000 }
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i]
    if (a === '-i' || a === '--input') out.input = argv[++i] ?? ''
    else if (a === '--prompt') out.prompt = argv[++i] ?? ''
    else if (a === '--timeout') out.timeout = Number(argv[++i]) || 90000
...
// 递增重试的每次预算：10s → 20s → 30s → …（总预算 90 秒封顶）
const RETRY_INTERVALS_MS = [10_000, 20_000, 30_000, 40_000, 50_000, 60_000]
...
async function main() {
  ...
  const engine = cfg?.providers?.openai
  if (!engine?.apiKey || !engine?.baseUrl || !engine?.model) {
  ...
  const capMs = Math.max(args.timeout, 90_000)
  let spent = 0
  const lastErrors = []

  for (const interval of RETRY_INTERVALS_MS) {
    if (spent >= capMs) break
    const budget = Math.min(interval, capMs - spent)
    spent += budget
    ...
    try {
      ...
      lastErrors.push(`attempt ${lastErrors.length + 1} (${budget / 1000}s budget): ${msg}`)
      console.error(`[modlens-fast] attempt ${lastErrors.length} failed (${budget / 1000}s budget): ${msg} — 递增重试中`)
      // 递增重试：继续下一档
    } finally {
      clearTimeout(timer)
    }
  ...
  console.error(`vision api failed after ${lastErrors.length} attempts:\n${lastErrors.slice(-4).join('\n')}`)
  process.exit(1)
}
